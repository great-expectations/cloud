...
class ExpectationService:
    def __init__(self, context: CloudDataContext, base_url: str, auth_key: str):
    ...
    def get_existing_expectations_by_data_asset(
        ...
        """Fetch all existing expectations for a data asset.

        Args:
            data_source_name: Filter by data source
            data_asset_name: Filter by data asset
            organization_id: Organization ID for the API call
            workspace_id: Workspace ID for the API call

        Returns:
            A list of structured expectation context objects, intended to be used as context for the LLM.

        Raises:
            ListExpectationsError: If the API request fails
        """
