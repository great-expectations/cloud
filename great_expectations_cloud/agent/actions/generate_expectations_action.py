...
from great_expectations_cloud.agent.actions.types import (
    CREATED_VIA_ECHOES,
...
from great_expectations_cloud.agent.expect_ai.asset_review_agent.state import EchoesInput
from great_expectations_cloud.agent.expect_ai.tools.query_runner import QueryRunner
from great_expectations_cloud.agent.models import CreatedResource, DomainContext, RunRdAgentEvent
...
from great_expectations_cloud.agent.services.metric_service import MetricService
...
class GenerateExpectationsAction(AgentAction[RunRdAgentEvent]):
    def __init__(  # Refactor opportunity
        self,
    ...
    @override
    def run(self, event: RunRdAgentEvent, id: str) -> ActionResult:
        ...
        asset_review_result = asyncio.run(agent.arun(echoes_input=echoes_input))
    ...
    def _batch_contains_no_rows(self, event: RunRdAgentEvent) -> bool:
        batch_definition = (
    ...
    def _create_expectation_draft_configs(
        self, id: str, event: RunRdAgentEvent, expectations: list[gxe.Expectation]
    ) -> ActionResult:
        draft_config_service = ExpectationDraftConfigService(
            context=self._context,
            created_via=CREATED_VIA_ECHOES,
        )
    ...
    def _add_expectations_to_managed_asset(
        ...
        for expectation in expectations:
            ...
            expectation_payload["created_via"] = CREATED_VIA_ECHOES
